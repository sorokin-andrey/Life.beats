...
import com.quickblox.lifebeats.ui.Registration;
...
public class LoginPage extends Activity {
    ...
    public void register_onClick(View v) {
        CommonFunctions.startNewActivity(LoginPage.this, Registration.class);
        ...
        AsyncTask<Void, Void, Boolean> task = new AsyncTask<Void, Void, Boolean>() {
                ...
                try {
                    ...
                    if (CommonFunctions.doesDatabaseExist(context, StaticVariables.databaseName)) {
                        ...
                        if (!user.getPassword().equals("")) {
                            String password = CommonFunctions.getStringFromEditText(LoginPage.this, etPassword);
                            if (user.getPassword().equals(password)) {
                                StaticVariables.currentUser = user;
                                CommonFunctions.startNewActivity(LoginPage.this, Main.class);
                                overridePendingTransition(R.anim.push_left_in, R.anim.push_left_out);
                                return true;
                            } else
                                return false;
            ...
            protected void onPostExecute(Boolean result) {
                if (pd != null) {
                    pd.dismiss();
                    if (!result) {
                        CommonFunctions.showAlert(LoginPage.this, "There is some error", "Please enter correct password. Password must contains 6 to 20 characters, one digit from 0-9, one lowercase and one uppercase character.");
                    }
                }
            }
        };
